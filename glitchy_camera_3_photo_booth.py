...
class GlitchySilhouetteProcessor:
    ...
    def __init__(self):
        ...
        self.use_filter_effects = True  # Toggle for glitch effects
        ...
        # Photo booth effect parameters
        self.head_effect_active = False
        self.head_effect_start_time = None
        self.head_effect_duration = 5.0  # 5 seconds countdown
    ...
    def process_frame(self, frame):
        ...
        if self.ready_for_detection and self.background_image is not None:
            ...
            # Step 7: Apply glitchy effects to silhouette areas (if enabled)
            if self.use_filter_effects:
                result = self.apply_glitch_effect(frame, soft_mask)
            else:
                result = frame.copy()
            
            # Step 8: Apply head effect if active
            ...
            # Step 10: Add debug info (if enabled)
    ...
    def draw_pose_landmarks(self, frame):
        ...
        landmark_color, connection_color = self.get_skeleton_colors()
    ...
    def add_debug_overlay(self, frame, contour_count, silhouette_pixels):
        ...
        if self.use_pose_estimation:
            ...
            if self.pose_detected:
                hands = []
                if self.left_hand_raised:
                    hands.append("L")
                if self.right_hand_raised:
                    hands.append("R")
                hand_status = "↑ " + "+".join(hands) if hands else "↓ Down"
                cv2.putText(frame, f"Hands: {hand_status}", (250, 135), font, 0.6, (0, 255, 255), 2)
                ...
                if self.head_effect_active and self.head_effect_start_time:
                    ...
                    cv2.putText(frame, f"📸 Photo: {remaining:.1f}s", (350, 135), font, 0.6, (0, 0, 255), 2)
        
        # Show toggle states
        toggles = f"M:{'ON' if self.use_pose_estimation else 'OFF'} F:{'ON' if self.use_filter_effects else 'OFF'} H:{'ON' if self.show_debug_info else 'OFF'}"
        cv2.putText(frame, toggles, (20, 160), font, 0.5, (200, 200, 200), 1)
        
        # Controls
        cv2.putText(frame, f"Controls: SPACE=Mode WASD=Sens/Intensity R=Reset MFH=Toggles", (20, 180), font, 0.5, (200, 200, 200), 1)
        
        # Show current sensitivity if available
        if self.bg_subtractor is not None:
            sensitivity = self.bg_subtractor.getVarThreshold()
            cv2.putText(frame, f"MOG2 Var: {sensitivity:.0f} | Intensity: {self.glitch_intensity:.1f}", (20, 200), font, 0.5, (255, 255, 0), 1)
    ...
    def apply_head_color_effect(self, frame, original_frame):
        ...
        # Take screenshots at the end (we'll implement this in the next version)
        if elapsed_time >= self.head_effect_duration - 0.1 and not self.effect_screenshots_taken:
            # Screenshots will be taken in v4
            self.effect_screenshots_taken = True
            print("📸 [Photo capture placeholder - will save in v4]")
        
        result = frame.copy()
        
        # Draw countdown box if pose is detected
        if self.pose_detected and self.pose_landmarks:
            # Get bounding box around the pose
            ...
            if bbox:
                ...
                # Position text at top center of bounding box
                text_x = x + (w - text_size[0]) // 2
                text_y = y - 10
                
                # Draw countdown number
                cv2.putText(result, countdown_text, (text_x, text_y), 
                           font, font_scale, (0, 0, 255), thickness)
                
                # Flash effect in the last 0.2 seconds
                if elapsed_time >= self.head_effect_duration - 0.2:
                    # White flash overlay
                    flash_overlay = np.ones_like(result) * 255
                    alpha = 0.7
                    result = cv2.addWeighted(result, 1 - alpha, flash_overlay, alpha, 0)
        
        return result
    
    def get_skeleton_bbox(self, frame_shape):
        """Get bounding box around detected skeleton with 10% padding"""
        if not self.pose_landmarks:
            return None
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Glitchy Silhouette Replacement with Camera Selection')
    ...
    while True:
        ...
        if key == ord('q') or key == 27:  # ESC
            ...
            processor.adjust_glitch_intensity(0.2)
        elif key == ord('m') or key == ord('M'):  # M = Toggle MediaPipe
            if MEDIAPIPE_AVAILABLE:
                processor.use_pose_estimation = not processor.use_pose_estimation
                print(f"🤖 MediaPipe: {'ON' if processor.use_pose_estimation else 'OFF'}")
            else:
                print("⚠️ MediaPipe no disponible")
        elif key == ord('f') or key == ord('F'):  # F = Toggle filter effects
            processor.use_filter_effects = not processor.use_filter_effects
            print(f"✨ Efectos: {'ON' if processor.use_filter_effects else 'OFF'}")
        elif key == ord('h') or key == ord('H'):  # H = Toggle debug info
            processor.show_debug_info = not processor.show_debug_info
            print(f"📊 Debug: {'ON' if processor.show_debug_info else 'OFF'}")
