...
class GlitchySilhouetteProcessor:
    ...
    def __init__(self):
        ...
        self.learning_duration = 5.0  # 5 seconds to learn background thoroughly
        self.learning_frames_needed = 150  # More frames to process for learning
        self.learning_frames_processed = 0
        
        # MOG2 Background Subtractor (will be initialized after countdown)
        self.bg_subtractor = None
        
        # Edge detection parameters (adaptive)
        self.blur_kernel = 5
        ...
        self.shadow_threshold = 0.6  # intensity ratio for shadow detection
    ...
    def process_frame(self, frame):
        ...
        if self.countdown_active:
            ...
            if remaining_time > 0:
                ...
                self.learning_frames_processed = 0
                # Initialize MOG2 now with better parameters for learning
                self.bg_subtractor = cv2.createBackgroundSubtractorMOG2(
                    history=500,           # More frames to learn background thoroughly
                    varThreshold=50,       # Start with lower sensitivity during learning
        ...
        if self.learning_background:
            # Feed frames to MOG2 for learning with HIGH learning rate
            if self.bg_subtractor is not None:
                # Use high learning rate during training phase
                self.bg_subtractor.apply(frame, learningRate=0.1)
                self.learning_frames_processed += 1
                if self.learning_frames_processed % 10 == 0:
                    print(f"Learning frame {self.learning_frames_processed}/{self.learning_frames_needed}")
        ...
        if self.ready_for_detection and self.bg_subtractor is not None:
            ...
            # Step 2: Adaptive edge detection on original frame
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            blurred = cv2.GaussianBlur(gray, (self.blur_kernel, self.blur_kernel), 0)
            edges = self.adaptive_canny_detection(blurred)
            
            # Step 2.5: Shadow suppression 
            if self.shadow_detection:
                fg_mask = self.suppress_shadows(frame, fg_mask)
            
            # Step 3: Combine background mask with edges (weighted blend)
            combined_mask = cv2.addWeighted(fg_mask, 0.7, edges, 0.3, 0)
            
            # Step 3.5: Temporal filtering for stability
            combined_mask = self.temporal_filter(combined_mask)
            ...
            min_contour_area = 5000  # Much larger minimum area - ignore small changes
    ...
    def adaptive_canny_detection(self, gray_image):
        """Adaptive Canny edge detection based on local image statistics"""
        if self.adaptive_canny:
            # Calculate adaptive thresholds based on image statistics
            median = np.median(gray_image)
            sigma = 0.33  # sensitivity factor
            lower_threshold = int(max(0, (1.0 - sigma) * median))
            upper_threshold = int(min(255, (1.0 + sigma) * median))
            
            # Ensure minimum threshold values for edge detection
            lower_threshold = max(lower_threshold, 30)
            upper_threshold = max(upper_threshold, 60)
            
            return cv2.Canny(gray_image, lower_threshold, upper_threshold)
        else:
            return cv2.Canny(gray_image, 50, 150)
    
    def suppress_shadows(self, frame, foreground_mask):
        """Remove shadows using HSV color space analysis"""
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        
        # Create shadow mask: lower intensity but similar hue/saturation
        shadow_mask = np.zeros(foreground_mask.shape, dtype=np.uint8)
        
        # Find pixels that might be shadows
        h, s, v = cv2.split(hsv)
        
        # Shadow detection: lower value (brightness) but maintained hue
        shadow_candidates = (v < (255 * self.shadow_threshold)) & (s > 30)
        
        # Remove shadow candidates from foreground mask
        clean_mask = foreground_mask.copy()
        clean_mask[shadow_candidates] = 0
        
        return clean_mask
    
    def temporal_filter(self, mask):
        """Apply temporal filtering to reduce flickering"""
        # Add current mask to history
        self.temporal_history.append(mask.copy())
        
        # Keep only recent frames
        if len(self.temporal_history) > self.temporal_window:
            self.temporal_history.pop(0)
        
        # If we have enough history, apply filtering
        if len(self.temporal_history) >= 3:
            # Average the masks for stability
            filtered_mask = np.zeros_like(mask, dtype=np.float32)
            for hist_mask in self.temporal_history:
                filtered_mask += hist_mask.astype(np.float32)
            
            filtered_mask = (filtered_mask / len(self.temporal_history)).astype(np.uint8)
